...
const Loja = ({ id, nome, avaliacao, tempoEntrega, distancia, imagem, precoEntrega }) => {
    const navigate = useNavigate();

    const navigateToMenu = (id) => {
        navigate({ pathname: "/menu", search: `?id=${id}` });
    };

    return (
        <div className="card h-100 d-inline-block partner-card" style={{ width: "45%" }}>
            <Row>
                ...
                <Col md={10}>
                    <Row style={{ paddingTop: "5px" }}>
                        ...
                        <Col md={2} className="partner-title" style={{ textAlign: "end" }}>
                            <svg
                            ...
                            <span className="rating">{decimalFormatter(avaliacao, 1)}&nbsp;</span>
                        </Col>
                    </Row>
                    <Row>
                        <div className="partner-info partner-text">
                            Lanchonete • {decimalFormatter(distancia, 2)} km
                        </div>
                    </Row>
                    <Row>
